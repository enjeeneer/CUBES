...
class CausalSelfAttention(nn.Module):
    """
    A multi-head masked self-attention module with projection as output. Taken from Janner (2021)
    (https://github.com/jannerm/trajectory-transformer) who in turn mirrored Kaparthy (2020)
    ...
    def __init__(self, cfg):
        ...
        # causal mask to ensure that attention is only applied to the left in the input sequence (triang. matrix of 1s)
        self.register_buffer("mask", torch.tril(torch.ones(cfg.block_size, cfg.block_size))
                                     .view(1, 1, cfg.block_size, cfg.block_size))

        # mask previous value estimates
        joined_dim = cfg.obs_dim + cfg.act_dim + 2
        self.mask.squeeze()[:, joined_dim-1::joined_dim] = 0 # set every joined_dim(th) element to 0
    ...
    def forward(self, x):
        """
        :param x: input tensor of shape (batch_size (B), sequence_length (T), embedding dimensionality (C))
        :return:
        """
        ...
        # causal self-attention; Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T). This gives us
...
class Block(nn.Module):
    """
    A transformer block i.e. the combination of self-attention, layer normalisation and linear layers.
    """
    def __init__(self, cfg):
        super().__init__()
        ...
        self.mlp = nn.Sequential(
            nn.Linear(cfg.n_embd, 4 * cfg.n_embd), # 4x n_embd taken from GPT-3 paper
            nn.GELU(),
...
class GPT(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.stop_token = cfg.vocab_size * cfg.transition_dim
        self.block_size = cfg.block_size
        self.observation_dim = cfg.observation_dim

        self.action_dim = cfg.action_dim
        self.transition_dim = cfg.transition_dim # state_dim + act_dim + reward_dim + rtg_dim
        self.action_weight = cfg.action_weight
        self.reward_weight = cfg.reward_weight
