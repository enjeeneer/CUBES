...
class ReplayBuffer(object):
    ...
    def __init__(self, cfg, obs_shape, action_shape):
        ...
        self.obses = np.empty((int(cfg.replay_buffer_capacity), obs_shape), dtype=np.float32)
        self.next_obses = np.empty((int(cfg.replay_buffer_capacity), obs_shape), dtype=np.float32)
