...
house_schedules = {os.path.basename(f).replace('.csv', ''): pd.read_csv(f, parse_dates=['UTC_Time']).set_index('UTC_Time')
                   for f in house_files}

# Create a dictionary to load all room schedules
room_schedules = {os.path.basename(f).replace('_schedule.csv', ''): pd.read_csv(f, parse_dates=['UTC_Time']).set_index('UTC_Time')
                  for f in room_files}
...
all_rooms = [
    'front_room', 'kitchen', 'backroom', 'hall_downstairs',
    'bedroom_1', 'bedroom_2', 'bedroom_3', 'bathroom', 'hall_upstairs'
]
...
week_slices = [(week_idx * minutes_per_week, (week_idx + 1) * minutes_per_week) for week_idx in range(num_days // days_per_week)]

# Generate synthetic schedules
for replicate in tqdm(range(num_schedules), desc="Generating synthetic schedules"):
    synthetic_schedule = pd.DataFrame(index=time_index, columns=all_rooms)  # Initialise with full time index and rooms

    # Process each week, selecting a random house for each week
    for week_idx, (week_start_idx, week_end_idx) in tqdm(enumerate(week_slices), total=len(week_slices), desc=f"Processing weeks for replicate {replicate + 1}", leave=False):
        ...
        if available_rooms == 'all':
            week_data = house_schedules[selected_house].iloc[week_start_idx:week_end_idx].reindex(columns=all_rooms)
            ...
            for room in all_rooms:
                if room in available_rooms and room in house_schedules[selected_house].columns:
                    # Use the room's schedule from the selected house for this week
                    week_data[room] = house_schedules[selected_house][room].iloc[week_start_idx:week_end_idx].values
                else:
                    ...
                    week_data[room] = room_schedules[room][random_column].iloc[week_start_idx:week_end_idx].values

            # Vectorised assignment of weekly data
            synthetic_schedule.iloc[week_start_idx:week_end_idx] = pd.DataFrame(week_data, index=time_index[week_start_idx:week_end_idx], columns=all_rooms)
    ...
    print(f"Synthetic house schedule saved to '{output_filename}' with optimised sampling for replicate {replicate + 1}.")
