...
def generate_synthetic_schedule(occupancy_dict, num_minutes_in_year):
    synthetic_schedule = []
    for minute in tqdm(
        range(num_minutes_in_year), desc="Generating Schedule", unit="min", leave=True
    ):
        ...
        occupancy_probability = occupancy_dict.get(
            time_key, 0
...
room_names = [
    ...
    "kitchen_schedule",
    "bedroom_3_schedule",
    "hall_upstairs_schedule",
    "bedroom_2_schedule",
    "hall_downstairs_schedule",
]

# Load probability distribution data from CSV file
print("Loading probabilities from CSV file...", flush=True)
final_probability_distribution = pd.read_csv(
    "/workspaces/CUBES/exp/jack/paper/thermostat_experiment/SI/generate_occupancy/final_probability_distributions_numeric.csv"
)
...
room_occupancy_dicts = {
    room_name: create_occupancy_dict(
        final_probability_distribution[
            final_probability_distribution["room"] == room_name
        ]
    )
...
for schedule_index in range(num_schedules):
    print(
        f"Starting schedule generation for repetition {schedule_index}...", flush=True
    )
    room_schedules = {}
    ...
    for room_name in room_names:
        ...
        room_schedule = generate_synthetic_schedule(
            room_occupancy_dicts[room_name], num_minutes_in_year
        )
    ...
    schedules_df.columns = [
        name.replace("_schedule", "") for name in schedules_df.columns
    ]

    # Step to fractionalise rows (Vectorised)
    row_sums = schedules_df.sum(axis=1)  # Compute the sum for each row
    schedules_df = schedules_df.div(row_sums, axis=0).fillna(
        0
    )  # Divide by row sum and handle division by zero
