...
class ReplayBuffer(object):
    ...
    def __init__(self, cfg):
        ...
        self.obses = np.empty(
            (int(cfg.replay_buffer_capacity), cfg.obs_dim), dtype=np.float32
        )
        self.next_obses = np.empty(
            (int(cfg.replay_buffer_capacity), cfg.obs_dim), dtype=np.float32
        )
        self.actions = np.empty(
            (int(cfg.replay_buffer_capacity), cfg.action_dim), dtype=np.float32
        )
        self.rewards = np.empty((int(cfg.replay_buffer_capacity), 1), dtype=np.float32)
        self.not_dones = np.empty(
            (int(cfg.replay_buffer_capacity), 1), dtype=np.float32
        )
        self.not_dones_no_max = np.empty(
            (int(cfg.replay_buffer_capacity), 1), dtype=np.float32
        )
