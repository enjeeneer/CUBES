...
class SACAgent(Agent):
    ...
    def __init__(self, cfg):
        ...
        self.log_alpha = torch.tensor(
            np.log(cfg.sac.init_temperature), dtype=torch.float32
        ).to(self.device)
        ...
        self.actor_optimizer = torch.optim.Adam(
            self.actor.parameters(), lr=cfg.sac.actor_lr, betas=cfg.sac.actor_betas
        )

        self.critic_optimizer = torch.optim.Adam(
            self.critic.parameters(), lr=cfg.sac.critic_lr, betas=cfg.sac.critic_betas
        )

        self.log_alpha_optimizer = torch.optim.Adam(
            [self.log_alpha], lr=cfg.sac.alpha_lr, betas=cfg.sac.alpha_betas
        )
    ...
    def update(self, replay_buffer, step):
        ...
        if step % self.cfg.sac.critic_target_update_frequency == 0:
            utils.soft_update_params(
                self.critic, self.critic_target, self.cfg.sac.critic_tau
            )
