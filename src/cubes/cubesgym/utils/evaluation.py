...
def evaluate_policy(
    ...
    :param env: The gym environment.
        In the case of a VecEnv this must contain only one environment.
    ...
    result = {
        ...
        "episodes_emissions_penalties": [],
        "episodes_air_quality_penalties": [],
    }
    episodes_executed = 0
    not_reseted = True
    while episodes_executed < n_eval_episodes:
        ...
        episode_emissions = 0.0
        episode_comfort_penalty = 0.0
        episode_emissions_penalty = 0.0
        episode_air_quality_penalty = 0.0
        # ---------------------------------------------------------------------------- #
        ...
        while not done:
            ...
            episode_emissions += info[0]["emissions"]
            episode_emissions_penalty += info[0]["reward_emissions"]
            episode_comfort_penalty += info[0]["reward_comfort"]
            episode_air_quality_penalty += info[0]["reward_air_quality"]
        ...
        result["episodes_emissions"].append(episode_emissions)
        try:
        ...
        result["episodes_emissions_penalties"].append(episode_emissions_penalty)
        result["episodes_air_quality_penalties"].append(episode_air_quality_penalty)
