...
class SoftActorCritic(AbstractAgent, metaclass=abc.ABCMeta):
    """
    ...
    def act(
        ...
        `transform_observation()` method unifies varying input types as tensors.
        The action is returned in real and neural space to enable action selection
        in production/simulation and logging in memory respectively. The transformed
        observation is returning for logging in memory too.
        Args:
            observation: Tensor, Dict or DataFrame
            sample: True if in training loop, False if in eval or production.
        Returns:
            action: Dict or DataFrame of action in real space
            neural_action: action array in neural space
        ...
        print("normalised observation", observation)
    ...
    def _update_actor(self, observations: torch.Tensor) -> Dict:
        """
    ...
    def _update_alpha(self, log_prob: torch.Tensor) -> Dict:
        """
