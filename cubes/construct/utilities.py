...
class ModifiedIDF(IDF):
    ...
    # def new_match_idf_surfaces(idf, storey_height=2.3, tol=0.01, debug_plot=False):
    #     """
    #     Match all surfaces in an IDF based on geometric equality.

    #     Rules:
    #         - Only matches surfaces that are coplanar (or offset by storey height for floors/ceilings).
    #         - Normals must be opposite (dot product ≈ -1).
    #         - Polygon shapes must match within tolerance (Shapely equality).
    #         - Horizontal matches: must be offset vertically (not same Z).
    #         - Vertical matches: same plane, opposite normals.

    #     Args:
    #         idf: Geomeppy IDF object
    #         storey_height (float): Expected vertical spacing between floors and ceilings.
    #         tol (float): Geometric tolerance for polygon equality.
    #         debug_plot (bool): If True, plots each matched pair for inspection.

    #     Returns:
    #         dict: {(surface1_name, surface2_name): (surface1, surface2)}
    #     """
    #     surfaces = idf.getsurfaces() + idf.getshadingsurfaces()
    #     matched = {}
    ...
    #         """Project 3D coords onto a 2D plane aligned with surface normal."""
    #         # Simple case: horizontal surfaces → project to XY
    #         if is_horizontal(normal):
    #             return [(x, y) for x, y, z in coords]

    #         # For vertical walls, create local axes
    ...
    #         return [(np.dot(np.array(c) - origin, u), np.dot(np.array(c) - origin, v)) for c in coords]
    ...
    #         # Orientation must match (both horizontal or both vertical)
    #         if is_horizontal(n1) != is_horizontal(n2):
    #             continue

    #         # Horizontal surfaces: skip if on same plane
    #         if is_horizontal(n1):
    #             z1 = np.mean([z for _, _, z in s1.coords])
    #             z2 = np.mean([z for _, _, z in s2.coords])
    #             z_diff = abs(z1 - z2)
    #             if z_diff < tol:  # same elevation → skip
    #                 continue
    #             # Optionally enforce expected floor-to-ceiling difference
    #             if abs(z_diff) > 0.2:  # allow some tolerance
    #                 continue

    #         # Project to 2D and compare
    #         poly1 = Polygon(project_to_plane(s1.coords, n1))
    #         poly2 = Polygon(project_to_plane(s2.coords, n2))
    ...
    # def new_match_idf_surfaces(idf, tol=0.01, debug_plot=False):
    #     """
    #     Match all surfaces in an IDF based on geometric equality.

    #     Rules:
    #         - Only matches surfaces that are coplanar.
    #         - Normals must be opposite (dot product < 0).
    #         - Polygon areas must be equal (within tolerance).
    #         - Uses Shapely to test equality of projected polygons.
    ...
    #     # Utility to classify orientation
    #     def is_horizontal(normal):
    #         return almostequal(abs(normal[2]), 1.0, 4)  # z-component ~ 1

    #     def normal_vector(coords):
    #         """Compute normal vector from surface coords (3D)."""
    #         v1 = np.array(coords[1]) - np.array(coords[0])
    #         v2 = np.array(coords[2]) - np.array(coords[0])
    #         n = np.cross(v1, v2)
    #         return n / np.linalg.norm(n)

    #     # Iterate pairs of surfaces
    #     for s1, s2 in product(surfaces, repeat=2):
    #         if s1 == s2:
    #             continue
    ...
    #         # Orientation filter (horizontal matches horizontal, vertical matches vertical)
    #         if is_horizontal(n1) != is_horizontal(n2):
    #             continue

    #         # Project to 2D polygon (drop one axis depending on orientation)
    #         if is_horizontal(n1):
    #             # Drop Z -> XY projection
    #             poly1 = Polygon([(x, y) for x, y, z in s1.coords])
    #             poly2 = Polygon([(x, y) for x, y, z in s2.coords])
    #         else:
    #             # Drop Y -> XZ projection if wall normal along Y, etc.
    #             # Simpler: project to plane perpendicular to normal using first coord offset
    #             # Here we assume vertical walls → drop Z → XY projection is still valid
    #             poly1 = Polygon([(x, y) for x, y, z in s1.coords])
    #             poly2 = Polygon([(x, y) for x, y, z in s2.coords])
    ...
    # def match_all_idf_surfaces(idf):
    #     surfaces = idf.getsurfaces() + idf.getshadingsurfaces()
    #     planes = getidfplanes(surfaces)
    #     matched = {}
