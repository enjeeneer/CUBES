...
class DatetimeWrapperCubes(gym.ObservationWrapper):
    ...
    def __init__(self, env: Any):
        ...
        # Update new shape
        new_shape = env.observation_space.shape[0] + 4
        ...
        self.variables["observation"].insert(day_index + 1, "weekday_sin")
        self.variables["observation"].insert(day_index + 2, "weekday_cos")
    ...
    def observation(self, observation: np.ndarray) -> np.ndarray:
        ...
        new_obs["weekday_cos"] = np.cos(2 * np.pi * (dt.weekday()) / 7)
        new_obs["weekday_sin"] = np.sin(2 * np.pi * (dt.weekday()) / 7)
