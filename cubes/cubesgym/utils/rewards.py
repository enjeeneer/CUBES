...
class LinearRewardTEAQEmissions(BaseReward):
    """This class implements a linear reward function based on
    temperature, emissions, and air quality

    It is the same as LinearRewardTEAQ but includes simulation information on energy
    cost
    """
    ...
    def __init__(
        ...
        It considers the total operational building cost,
        the absolute difference to temperature comfort for occupied zones,
        and the air quality in occupied zones.
        ...
        self.w_air_quality = air_quality_weight
        self.w_temperature = temperature_weight
        self.lambda_emissions = lambda_emissions  # remove
        self.lambda_cost = lambda_cost
        self.lambda_temp = lambda_temperature
        self.lambda_air_quality = lambda_air_quality
        self.negative_emissions_for_export = negative_emissions_for_export
        self.timesteps_per_hour = timesteps_per_hour
        self.emissions_reward_avg_n_timesteps = (
            emissions_reward_avg_n_timesteps  # remov
        )
        self.emissions_history = np.zeros(emissions_reward_avg_n_timesteps)  # remove
        self.thermal_comfort_bonus = thermal_comfort_bonus
        self.thermal_comfort_constant_penalty = thermal_comfort_constant_penalty
        self.air_quality_bonus = air_quality_bonus
        self.potential_based_shaping = potential_based_shaping
    ...
    def __call__(self) -> Tuple[float, Dict[str, Any]]:
        ...
        reward_terms = {
            "reward_emissions": self.w_emissions * reward_emissions,
            "reward_cost": self.w_cost * reward_cost,
            "reward_comfort": self.w_temperature * reward_comfort,
            "reward_air_quality": self.w_air_quality * reward_air_quality,
            "emissions": emissions,
            "cost": cost,
...
class LinearRewardTEAQCOST(BaseReward):
    """This class implements a linear reward function based on
    temperature, energy cost, and air quality"""
    ...
    def __call__(self) -> Tuple[float, Dict[str, Any]]:
        ...
        # Emissions term (unused but kept for analysis)
        self.emissions_history[0] = self._get_emissions(obs_dict)
        reward_emissions = -self.lambda_emissions * np.mean(self.emissions_history)
        emissions = self.emissions_history[0]
