...
class ConstantTemperature(BaseControl):
    ...
    def act(
        ...
        **kwargs
        ...
        """
        print("constant temp input action dict:", action_dict)
        for zone in self.zone_names:
            action_dict[self.temp_control_names[zone]] = self.temp_setpoint
            print("constant temp input updated action dict:", action_dict)
...
class DOca2014ThermostatControl(BaseControl):
    ...
    def __init__(self, user_type="random"):
        ...
        if self.user_type == "active":
            self.night_up = -4.29
            self.morning_up = 3.66
            self.day_up = 3.45
            self.afternoon_up = 3.42
            self.evening_up = 2.14
            self.rh_in_up = -0.09
            self.t_out_up = -0.14

            self.int_down = -3.51
            self.solar_rad_down = -0.02
